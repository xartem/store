...
@section('content')
    <x-forms.base-auth-form :title="'Регистрация'" action="{{ route('register.handle') }}" method="POST">
        @csrf

        <x-forms.text-input name="name" type="text" placeholder="Имя" required :is-error="$errors->has('name')" value="{{ old('name') }}"/>
        @error('name')<x-forms.error>{{ $message }}</x-forms.error>@enderror
        <x-forms.text-input name="email" type="email" placeholder="E-mail" required :is-error="$errors->has('email')" value="{{ old('email') }}" />
        ...
        <x-slot:socialAuth>
            <ul class="space-y-3 mt-3">
                <li>
                    <a href="{{ route('social-auth', 'github') }}" class="relative flex items-center h-14 px-12 rounded-lg border border-[#A07BF0] bg-white/20 hover:bg-white/20 active:bg-white/10 active:translate-y-0.5">
