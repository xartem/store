...
@section('content')
                ...
                <div class="grow flex flex-col lg:py-8">
                    ...
                    <ul class="sm:max-w-[360px] space-y-2 mt-8">
                        @if($product->json_properties)
                            @foreach($product->json_properties as $title => $property)
                                <li class="flex justify-between text-body"><strong class="text-white">{{ $title }}:</strong> {{ $property }}</li>
                            @endforeach
                        @endif
                    </ul>
                    ...
                    <form class="space-y-8 mt-8">
                        <div class="grid grid-cols-2 md:grid-cols-3 2xl:grid-cols-4 gap-4">
                            @foreach($options as $option_title => $option_values)
                                <div class="flex flex-col gap-2">
                                    <label for="filter-item-1" class="cursor-pointer text-body text-xxs font-medium">{{ $option_title }}</label>
        ...
        <section class="mt-16 xl:mt-24">
            ...
            <div class="products grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-x-8 gap-y-8 lg:gap-y-10 2xl:gap-y-12">
                @each('product.partials.view', $views, 'product')
            </div>
        </section>
@endsection