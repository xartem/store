    ...
    <section class="mt-16 lg:mt-24">
        ...
        <div class="flex flex-col lg:flex-row gap-12 lg:gap-6 2xl:gap-8 mt-8">
            ...
            <aside class="basis-2/5 xl:basis-1/4">
                <form action="{{ catalog_filter_url($category, ['sort' => request('sort')]) }}" class="overflow-auto max-h-[320px] lg:max-h-[100%] space-y-10 p-6 2xl:p-8 rounded-2xl bg-card">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div class="flex items-center gap-4">
                        <div class="flex items-center gap-2">
                            <a href="{{ catalog_filter_url($category, ['view_type' => 'grid']) }}" class="@if(is_catalog_view_type('grid')) pointer-events-none text-pink @endif inline-flex items-center justify-center w-10 h-10 rounded-md bg-card">
                            ...
                            <a href="{{ catalog_filter_url($category, ['view_type' => 'list']) }}" class="@if(is_catalog_view_type('list')) pointer-events-none text-pink @endif inline-flex items-center justify-center w-10 h-10 rounded-md bg-card text-white hover:text-pink">
                                <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 52 52">
                    ...
                    <div x-data="{sort: '{{ catalog_filter_url($category, ['sort' => request('sort')]) }}'}" class="flex flex-col sm:flex-row sm:items-center gap-3">
                        <span class="text-body text-xxs sm:text-xs">Сортировать по</span>
                            <select
                                ...
                                <option value="{{ catalog_filter_url($category, ['sort' => '']) }}" class="text-dark">по умолчанию</option>
                                <option value="{{ catalog_filter_url($category, ['sort' => 'price']) }}" class="text-dark">от дешевых к дорогим</option>
                                <option value="{{ catalog_filter_url($category, ['sort' => '-price']) }}" class="text-dark">от дорогих к дешевым</option>
                                <option value="{{ catalog_filter_url($category, ['sort' => 'title']) }}" class="text-dark">по наименованию</option>
                            </select>
                    </div>
                </div>

                <!-- Products list -->
                @if(is_catalog_view_type('list'))
                    <div class="products grid grid-cols-1 gap-y-8">
                        @each('catalog.partials.product-list', $products, 'product')
                    </div>
                @else
                    <div class="products grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-x-6 2xl:gap-x-8 gap-y-8 lg:gap-y-10 2xl:gap-y-12">
                        @each('catalog.partials.product-grid', $products, 'product')
                    </div>
                @endif
